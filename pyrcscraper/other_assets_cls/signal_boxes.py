...
from pyrcscraper import other_assets
from pyrcscraper.utils import get_cls_catalogue, get_last_updated_date, load_pickle, parse_table, parse_tr, save_pickle
...
class SignalBoxes:
    ...
    def cdd_sig_box(*directories):
        path = other_assets.cdd("Other assets", "Signal boxes")
        for directory in directories:
            path = os.path.join(path, directory)
        return path

    # Scrape signal box prefix codes for the given 'key word' (e.g. a starting letter)
    def scrape_signal_box_prefix_codes(self, keyword, update=False):
        """
        :param keyword: [str]
        :param update: [bool]
        :return:
        """
        path_to_file = self.cdd_sig_box("A-Z", keyword.title() + ".pickle")
    ...
    def get_signal_box_prefix_codes(self, update=False):
        """
        :param update: [bool]
        :return:
        """
        pickle_filename = "Signal-box-prefix-codes.pickle"
        path_to_pickle = self.cdd_sig_box(pickle_filename)
        if os.path.isfile(path_to_pickle) and not update:
            signal_box_prefix_codes = load_pickle(path_to_pickle)
        else:
            # Get every data table
            data = [self.scrape_signal_box_prefix_codes(i, update) for i in string.ascii_lowercase]
    ...
    #
    def scrape_non_national_rail_codes(self):
    ...
    #
    def get_non_national_rail_codes(self, update=False):
        ...
        if os.path.isfile(path_to_pickle) and not update:
            ...
            try:
                non_national_rail_codes = self.scrape_non_national_rail_codes()
