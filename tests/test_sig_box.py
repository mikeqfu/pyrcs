...
if __name__ == '__main__':
    ...
    print(f"\n  {signal_boxes.Key}:\n    {type(signal_box_prefix_codes1_)}, "
          f"{signal_box_prefix_codes1_.shape}")
    print("\n  {}: {}".format(signal_boxes.LUDKey,
                              signal_box_prefix_codes1[signal_boxes.LUDKey]))
    #   Signal boxes:
    ...
    non_national_rail_codes_data1_ = \
        non_national_rail_codes_data1[signal_boxes.NonNationalRailKey]
    ...
    print("\n  {}: {}".format(signal_boxes.LUDKey,
                              non_national_rail_codes_data1[signal_boxes.LUDKey]))
    #  Last updated date: xxxx-xx-xx

    non_national_rail_codes_data2 = signal_boxes.fetch_non_national_rail_codes(
        update=True, pickle_it=True, data_dir=test_data_dir)
    non_national_rail_codes_data2_ = \
        non_national_rail_codes_data2[signal_boxes.NonNationalRailKey]
    print("\n  {} data is consistent:".format(signal_boxes.NonNationalRailKey),
          all((x == y) for x, y in
              zip(non_national_rail_codes_data1_, non_national_rail_codes_data2_)))
